...
class Quad_point():
    """Class to represent a quadrature point with its coordinates and weight."""
    def __init__(self, point: list, weight: float):
        self.point = point
        self.weight = weight

# Quadrature points for 2D integration of triangles
...
class ConstBasisFunction():
    """Class to represent a constant basis function on a triangle."""
...
class RandomFieldV():
    """Class to represent the random field V with eigenvalues and eigenvectors."""
    ...
    def __call__(self, x: np.array, xi: np.array) -> tuple[float, float]:
    ...
    def find_supported_basis_function(self, x: np.array) -> int:
        """Find the index of the only supported basis function for a given point x."""
...
class DerivativeCovarianceExpression(fe.UserExpression):
    """Class to represent the derivative of the covariance function."""
    def __init__(self, f: callable, x: np.array, **kwargs):
        super().__init__(**kwargs)
        self.f = f
        self.x = x

    def eval(self, values, y: np.array):
        values[0] = self.f(self.x[0], self.x[1], y[0], y[1])

    def value_shape(self):
        return ()
    
def quad_tri(f: callable, basis_function: ConstBasisFunction) -> float:
    """Calculate the integral of a function over a triangle using quadrature."""
...
def jacob_get_c_entry(f: callable, basis_function_l: ConstBasisFunction, basis_function_k: ConstBasisFunction) -> float:
    """Calculate the entry of the matrix C in the extended eigenvalue problem."""
...
def jacob_get_C_matrices(basis_functions: list[ConstBasisFunction]) -> list[np.array]:
    """Calculate the C matrices for the extended eigenvalue problem."""
...
class JacobianV():
    def __init__(self, eigenvalues: np.array, eigenvectors: np.array, basis_functions: list[ConstBasisFunction], N: int, J: int):
        """Class to represent the Jacobian V with eigenvalues and eigenvectors.
        ...
        Args: 
            ...
            J: Karhunen-Loève truncation level.
        """
        self.eigenvalues = eigenvalues
        self.eigenvectors = eigenvectors
        self.basis_functions = basis_functions
...
class JacobianVFixedXi():
    """Class to represent the Jacobian V with fixed xi."""
...
class AExpression(fe.UserExpression):
    """Class to represent the A matrix in the weak form of the domain-mapped Poisson equation."""
    def __init__(self, jacobianV_fixed_xi: JacobianVFixedXi, **kwargs):
        super().__init__(**kwargs)
...
class detJExpression(fe.UserExpression):
    """Class to represent the determinant of the Jacobian matrix."""
    def __init__(self, jacobianV_fixed_xi: JacobianVFixedXi, **kwargs):
        super().__init__(**kwargs)
        self.jacobianV_fixed_xi = jacobianV_fixed_xi
...
class RandomFieldExpression(fe.UserExpression):
    """Class to represent the random field V as a UserExpression."""
...
def get_C_entry(f: callable, basis_function_i: ConstBasisFunction, basis_function_j: ConstBasisFunction) -> float:
    """Calculate the entry of the matrix C in the extended eigenvalue problem."""
...
def calculate_vector_field_eigenpairs(mesh_resolution: int) -> tuple[RandomFieldV, JacobianV]:
    """Calculate the eigenpairs of the covariance kernel and return the random field V and Jacobian V.
    Solves the extended eigenvalue problem for the covariance kernel.

    Args:
        mesh_resolution: The resolution of the mesh.
    Returns:
        tuple: A tuple containing the random field V and Jacobian V.
    """
...
def solve_poisson_for_given_sample(fem_mesh_resolution: int, jacobianV: JacobianV, xi: np.array, f: float) -> fe.Function:
    """Solve the Poisson equation for a given sample of the random field V.
    
    Args:
        fem_mesh_resolution: The resolution of the finite element mesh.
        jacobianV: The Jacobian V object.
        xi: The random variables for the random field V.
        f: The right-hand side function of the Poisson equation.
    Returns:
        fe.Function: The solution of the Poisson equation.
    """
...
def save_results_to_csv(u_sols_evaluated: np.array, xis: np.array, fem_res: int, kl_res: int) -> None:
    """Save the evaluated solutions and random variables to CSV files.
    
    Args: 
        u_sols_evaluated: The evaluated solutions.
        xis: The random variables.
        fem_res: The finite element mesh resolution.
        kl_res: The Karhunen-Loève mesh resolution.
    """
    i = 0
...
def calculate_samples_and_save_results(mc_samples: int, fem_res: int, kl_res: int, randomFieldV: RandomFieldV = None, jacobianV: JacobianV = None) -> None:
    """Calculate samples and save the results of the Poisson equation to CSV files for Monte Carlo Analysis.
    
    Args: 
        mc_samples: The number of Monte Carlo samples.
        fem_res: The finite element mesh resolution.
        kl_res: The Karhunen-Loève mesh resolution.
        randomFieldV: Optional, the random field V object.
        jacobianV: Optional, the Jacobian V object.
    """
...
def analyse_two_resolutions_from_data_u_hat(resolution_sparse: int, resolution_fine: int, P_hat: fe.Point) -> None:
    """MC Analysis of the results of the Poisson equation for two different mesh resolutions.
    
    Args:
        resolution_sparse: FEM mesh resoluton and sparse KL mesh resolution.
        resolution_fine: The fine KL mesh resolution.
        P_hat: The point at which the solution is evaluated.
    """
...
def plot_mesh(mesh_resolution: int, randomFieldV: RandomFieldV) -> None:
    """Plot the original and perturbed mesh."""
...
def incoherence_fe_edges_visualisation(randomFieldV: RandomFieldV, mesh_resolution: int) -> None:
    """Visualize the original and mapped points in the mesh."""
...
def inverse_mapping(P: fe.Point, randomFieldV: RandomFieldV, xi: np.array, mesh_resolution_inverse_mapping: int) -> fe.Point:
    """Perform inverse mapping to find the point P_hat in the original mesh."""
...
def barycentric_coords(P: fe.Point, vertices: np.array) -> np.array:
    """Calculate the barycentric coordinates of point P with respect to the triangle defined by vertices."""
    A = np.array([
...
def poisson_sobol_run_samples_and_save(mc_sample_size: int, fem_res: int, kl_res: int, size_of_xi: int, randomFieldV: RandomFieldV = None, jacobianV: JacobianV = None) -> None:
    """Run Sobol sampling and save the resulting matrices in .npy files.
    
    Args:
        mc_sample_size: The number of Monte Carlo samples.
        fem_res: The finite element mesh resolution.
        kl_res: The Karhunen-Loève mesh resolution.
        size_of_xi: The size of the random variable xi.
        randomFieldV: Optional, the random field V object.
        jacobianV: Optional, the Jacobian V object.
    """
    if not randomFieldV or not jacobianV:
...
def poisson_sobol_calc_indices_from_data(fem_res: int, kl_res: int, size_of_xi: int, randomFieldV: RandomFieldV = None, jacobianV: JacobianV = None) -> tuple[np.array, np.array, int]:
    """Calculate Sobol indices from the saved data.
    ...
    Args:
        ...
        jacobianV: Optional, the Jacobian V object.
    Returns:
        tuple: The first-order and total Sobol indices, and the number of Monte Carlo samples."""
    if not randomFieldV or not jacobianV:
        randomFieldV, jacobianV = calculate_vector_field_eigenpairs(kl_res)
...
def poisson_plot_sobols(S_single: np.array, S_total: np.array, mc_sample_size: int, title: str) -> None:
