...
def double_loop_mc(mc_sample_size, mesh_resolution_fem, P, indices, randomFieldV, jacobianV, size_of_xi, test = False):
        ...
        for k in range(mc_sample_size):
            for index in missing_indices:
                if index < size_of_xi:
                    ...
                    F_samples[index - size_of_xi] = np.random.random(size=1)
            if test:
                sols_point_P.append(test_f(np.concatenate((xi, F_samples))))
            else:
...
def pick_freeze(mc_sample_size, mesh_resolution_fem, P, indices, randomFieldV, jacobianV, size_of_xi, test = False):
    # Translate math indices to CS indices
    indices = [index - 1 for index in indices]
    ...
    for i in range(mc_sample_size):
        ...
        if test:
            y.append(test_f(np.concatenate((xi, F_samples))))
            y_tilde.append(test_f(np.concatenate((xi_prime, F_samples_prime))))
        else:
            P_hat = inverse_mapping(P, randomFieldV, xi, mesh_resolution_fem)
...
def rank_statistics(mc_sample_size, mesh_resolution_fem, P, index, randomFieldV, jacobianV, size_of_xi, test = False):
    ...
    for i in range(mc_sample_size):
        ...
        if test:
            y.append(test_f(np.concatenate((xis[i, :], F_samples[i, :]))))
        else:
